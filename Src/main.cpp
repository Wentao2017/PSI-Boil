...
main(int argc, char * argv[]) {
  ...
  for(time.start(); time.end(); time.increase()) {
    ...
    for_vijk(c,i,j,k){
      real sum_flux = (-uvw[Comp::u()][i][j][k] + uvw[Comp::u()][i+1][j][k]) * c.dSx(i,j,k)
                    + (-uvw[Comp::v()][i][j][k] + uvw[Comp::v()][i][j+1][k]) * c.dSy(i,j,k)
                    + (-uvw[Comp::w()][i][j][k] + uvw[Comp::w()][i][j][k+1]) * c.dSz(i,j,k);
      if(k==1&&(i!=c.si()&&i!=c.ei()&&j!=c.sj()&&j!=c.ej())) 
        if(std::fabs(sum_flux)>boil::pico)
        std::cout<<"sum_flux="<<sum_flux/c.dV(i,j,k)<<" "<<i<<" "<<j<<" "<<k<<"\n";
    }
#endif 
